...
public class FlyGear3D implements GLEventListener {
    ...
    public void display(GLAutoDrawable dr) { // clear the screen and draw the box
        ...
        for (int i = 0; i < 10; i++) {
            float frac = i / 10f;
            gl2.glPushMatrix();
            gl2.glColor3f(0, 0, 1 - frac);
            gl2.glRotated(angle * i, 0, 1, 0);
            gl2.glTranslated(0, 0, -85);
            gl2.glScaled(50, 25, 50);
            drawTeeth(gl2);
            gl2.glPopMatrix();
        }
    ...
    private void drawTeeth(GL2 gl2) {
        gl2.glPushMatrix();
        gl2.glTranslated(-0.25, 0, -0.25);
