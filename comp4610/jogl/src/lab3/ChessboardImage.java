...
public class ChessboardImage implements GLEventListener {
    ...
    public void display(GLAutoDrawable dr) {
        ...
        float bottomX = -texture.getWidth() / 2f;
        float bottomY = -texture.getHeight() / 2f;

        TextureCoords textureCoords = texture.getImageTexCoords();
        gl.glTexCoord2f(textureCoords.left(), textureCoords.bottom());
        gl.glVertex2f(bottomX, bottomY);
        gl.glTexCoord2f(textureCoords.right(), textureCoords.bottom());
        gl.glVertex2f(bottomX + texture.getWidth(), bottomY);
        gl.glTexCoord2f(textureCoords.right(), textureCoords.top());
        gl.glVertex2f(bottomX + texture.getWidth(), bottomY + texture.getHeight());
        gl.glTexCoord2f(textureCoords.left(), textureCoords.top());
        gl.glVertex2f(bottomX, bottomY + texture.getHeight());
    ...
    public void init(GLAutoDrawable dr) {
        ...
        try {
            texture = TextureIO.newTexture(new File("chessimage.png"), true);
        } catch(Exception e) {
            e.printStackTrace();
        }
    }
