	...
	public static void main(String[] args) throws InterruptedException {
		Runnable run = new Runnable() {
			...
			public void run() {
				...
				Line line = new Line() {
					void plotLineLow(BufferedImage buf, int x0, int y0, int x1, int y1, int rgb) {
						int dx = x1 - x0;
						int dy = y1 - y0;
						int yi = 1;
					...
					void plotLineHigh(BufferedImage buf, int x0, int y0, int x1, int y1, int rgb) {
						int dx = x1 - x0;
						int dy = y1 - y0;
						int xi = 1;
...
//						}
						...
						if (Math.abs(xn - x0) > Math.abs(yn - y0)) {
							if (x0 < xn) plotLineLow(buf, x0, y0, xn, yn, rgb);
							else plotLineLow(buf, xn, yn, x0, y0, rgb);
						} else {
							if (y0 < yn) plotLineHigh(buf, x0, y0, xn, yn, rgb);
							else plotLineHigh(buf, xn, yn, x0, y0, rgb);
						}
